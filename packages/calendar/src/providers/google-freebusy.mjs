...
function createGoogleFreeBusyProvider({
  calendarId = 'primary',
  ...
  return {
    ...
    async getBusy({ start, end, timezone }) {
      ...
      if (!response.ok) {
        throw new GoogleCalendarProviderError('GOOGLE_FREEBUSY_FAILED', `Google Calendar FreeBusy failed with HTTP ${response.status}`);
      }
